...
let added = ref(false)
function addBobaToCart(bobaItem: {
    id: number,
    title?: string,
    price?: number,
    bobaImg?: string,
    ...
    bobaItem.id = boba.id;
    bobaItem.title = boba.title;
    
    store.addToCart(bobaItem)
...
<template >
    <div>
        ...
        <button v-if="!added" @click="(() => added = true)">Add to cart {{ totalPrice }} SEK</button>
        <button :class="added ? 'expanded' : 'animated-button'" :style="{ transition: '850ms' }"
